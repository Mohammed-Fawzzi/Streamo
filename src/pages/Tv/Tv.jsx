...
import { allTv, useMovies } from "@/hooks/useMovies";
import { getTvVideos } from "@/hooks/useVideos";
...
const Tv = () => {
  ...
  const { data, isLoading, isError, error } = useMovies("Tv", allTv);
  const { searchTerm, handleSearch, filteredData } = useSearch(
    data,
    "original_name"
  );

  const handleWatchNow = useCallback(async (tvId) => {
    try {
